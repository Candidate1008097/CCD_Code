...
def solve_ec2e(alpha, ka_0, kc_0, k, E, E0a, E0c, delta_t, delta_x, t_max, x_max, save, label, force_calculation=False, verbose=False):
    ...
    # finite difference matrix (changes for different t and B)
    def f_d_matrix(t_n, B_n):
        # exponents
        ...
        main_B_diagonal = 1 + 2*k*delta_t*B_n + 2*delta_t/(delta_x**2)  # changed from ECE
        main_B_diagonal[0] += 2*(delta_t/delta_x)*ka_0*exp2a  # changed from ECE
        main_B_diagonal[-1] = 1
        ...
        C_B_diagonal = -k*delta_t*B_n  # changed from ECE
        C_B_diagonal[-1] = 0
