...
def solve(alpha, k_0, E, E0, u_0, u_Inf, delta_t, delta_x, t_max, x_max, save, label, force_calculation=False,
    ...
    def f_d_matrix(t_n):
        ...
        matrix[0, 0] = 1 + 2 * r * (1 + delta_x * k_0 * (math.exp((1 - alpha) * (E(t_n) - E0)) + math.exp(-alpha * (E(t_n) - E0))))
        matrix[0, 1] = -2 * r
        matrix[-1, -1] = 1
        matrix[-1, -2] = 0
        return matrix
...
def current(tts, xxs, U):
    ...
    h_1 = xs[2] - xs[1]  # usually the same as h_0, but included in case we want to use it later for different spacings
