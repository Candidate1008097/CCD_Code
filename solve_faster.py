...
def solve(alpha, k_0, E, E0, u_0, u_Inf, delta_t, delta_x, t_max, x_max, save, label, force_calculation=False,
    ...
    def f_d_matrix(t_n):
        # form symbolic matrix
        t = sym.Symbol('t')
        Et = E(t)
        Etm = Et - E0
        hpad = sym.Matrix(0, 1, [])
        vpad = sym.Matrix(1, 0, [])
        matrix = - r * sym.diag(vpad, sym.eye(N - 1), hpad) - r * sym.diag(hpad, sym.eye(N - 1), vpad) + (1 + 2 * r) * sym.eye(N)
        matrix[0, 0] = 1 + 2 * r * (1 + delta_x * k_0 * (sym.exp((1 - alpha) * Etm) + sym.exp(-alpha * Etm)))
        matrix[0, 1] = -2 * r
        matrix[-1, -1] = 1
        matrix[-1, -2] = 0
        ...
        matrix = sym.lambdify(t, matrix, 'numpy')
    ...
    for j in range(J - 1):
        if verbose:
            print('Solving timestep {} of {}.'.format(j + 1, J - 1))
        # solve concentration
        U[j + 1, :] = np.linalg.solve(f_d_matrix(ts[j + 1]), U[j, :] + 2 * r * delta_x * k_0 * math.exp(-alpha * (E(ts[j + 1]) - E0)) * np.eye(1, N).reshape(-1))
